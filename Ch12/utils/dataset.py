...
def parse_aug_fn(dataset, input_size=(416, 416)):
    ...
    # 將(x1, y1, x2, y2)正規化
    y = tf.divide(y, [ih, iw, ih, iw, 1])
    # 將(num_boxes, [x1, y1, x2, y2, classes])擴充到(100, [x1, y1, x2, y2, classes])大小
    paddings = [[0, 100 - tf.shape(y)[0]], [0, 0]]
    y = tf.pad(y, paddings)
    # 重新定義y輸出的shape
    y = tf.ensure_shape(y, (100, 5))
...
def transform_targets(x_train, y_train, anchors, anchor_masks, grid_size=13):
    ...
    # y_train: (batch_size, 100, [x1, y1, x2, y2, classes])
    # anchor_idx: (batch_size, 100, best_anchor)
    # 合併y_train和anchor_idx -> (batch_size, 100, [x1, y1, x2, y2, classes, best_anchor])
